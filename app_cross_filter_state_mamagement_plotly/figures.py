...
def apply_filters(df: pd.DataFrame, selections: dict, ignore_dims: list = None) -> pd.DataFrame:
    """
    ...
    if "year" not in ignore_dims and sel_year != "All":
        try:
            # Handle potential string vs int mismatch
            y = int(sel_year)
            d = d[d["Intake Year"] == y]
...
def get_kpis(df: pd.DataFrame) -> dict:
    """
    Calculates summary statistics (KPIs) for the provided dataframe.
    """
    if df.empty:
    ...
    # Calculate LOS (Check if column exists first to be safe)
    avg_los = 0.0
    if "intake_duration" in df.columns and total_intakes > 0:
        avg_los = float(df["intake_duration"].mean())
...
# =============================================================================
# FIGURE GENERATORS (VISUALIZATION LAYER)
# =============================================================================
...
def create_intake_bar(df: pd.DataFrame, selected="All") -> go.Figure:
    ...
    colors = ["#0d6efd" if (selected == "All" or x == selected) else "#e9ecef" for x in counts["Intake Type"]]
...
def create_trend_chart(df: pd.DataFrame) -> go.Figure:
    ...
    trend_in = df.groupby("Intake Year").size().reset_index(name="Count")
    trend_out = df.groupby("Outcome Year").size().reset_index(name="Count")
    
    trend_in = trend_in.dropna(subset=["Intake Year"])
    trend_out = trend_out.dropna(subset=["Outcome Year"])
    
    if trend_in.empty and trend_out.empty: return _no_data_fig()
    ...
    fig.add_trace(go.Scatter(
        ...
        stackgroup="one", fillcolor="rgba(25,135,84,0.2)"
    ))
    fig.add_trace(go.Scatter(
        x=trend_in["Intake Year"].astype(int).astype(str),
        y=trend_in["Count"],
        name="Intake", mode="lines+markers",
...
def create_outcome_bar(df: pd.DataFrame, selected="All") -> go.Figure:
    ...
    colors = ["#198754" if (selected == "All" or x == selected) else "#e9ecef" for x in counts["Outcome Type"]]
